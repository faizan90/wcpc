...
def write_gen_mod_cp_rules_lines(params_dict):
    ...
    pyxcd.w('cdef int ret_cont_flag(')
    pyxcd.ind()
    pyxcd.w('DT_UL_NP_t[:, :] cp_rules,')
    pyxcd.w('const np.uint8_t[:, :] anom_crnr_flags_arr,')
    pyxcd.w('const DT_UL n_anom_cols,')
    pyxcd.w('const DT_UL n_fuzz_nos,')
    pyxcd.w('const int thresh_steep,')
    pyxcd.w('const DT_UL cp_no,')
    pyxcd.w('const DT_UL pt_no,')
    pyxcd.w('const DT_UL fuzz_no,')
    pyxcd.w(') nogil:')
    pyxcd.els()

    pyxcd.w('cdef:')
    pyxcd.ind()
    pyxcd.w('int curr_rule')
    pyxcd.ded()

    pyxcd.w('if anom_crnr_flags_arr[pt_no, 0]:')
    pyxcd.ind()
    pyxcd.w('curr_rule = <int> cp_rules[cp_no, pt_no - n_anom_cols - 1]')
    pyxcd.w('if (curr_rule != n_fuzz_nos) and (abs(<int> (fuzz_no - curr_rule)) > thresh_steep):')
    pyxcd.ind()
    pyxcd.w('return 1')
    pyxcd.ded(lev=2)

    pyxcd.w('if anom_crnr_flags_arr[pt_no, 1]:')
    pyxcd.ind()
    pyxcd.w('curr_rule = <int> cp_rules[cp_no, pt_no - n_anom_cols]')
    pyxcd.w('if (curr_rule != n_fuzz_nos) and (abs(<int> (fuzz_no - curr_rule)) > thresh_steep):')
    pyxcd.ind()
    pyxcd.w('return 1')
    pyxcd.ded(lev=2)

    pyxcd.w('if anom_crnr_flags_arr[pt_no, 2]:')
    pyxcd.ind()
    pyxcd.w('curr_rule = <int> cp_rules[cp_no, pt_no - n_anom_cols + 1]')
    pyxcd.w('if (curr_rule != n_fuzz_nos) and (abs(<int> (fuzz_no - curr_rule)) > thresh_steep):')
    pyxcd.ind()
    ...
    pyxcd.w('if anom_crnr_flags_arr[pt_no, 3]:')
    pyxcd.ind()
    pyxcd.w('curr_rule = <int> cp_rules[cp_no, pt_no - 1]')
    pyxcd.w('if (curr_rule != n_fuzz_nos) and (abs(<int> (fuzz_no - curr_rule)) > thresh_steep):')
    pyxcd.ind()
    pyxcd.w('return 1')
    pyxcd.ded(lev=2)

    pyxcd.w('if anom_crnr_flags_arr[pt_no, 4]:')
    pyxcd.ind()
    pyxcd.w('curr_rule = <int> cp_rules[cp_no, pt_no + 1]')
    pyxcd.w('if (curr_rule != n_fuzz_nos) and (abs(<int> (fuzz_no - curr_rule)) > thresh_steep):')
    pyxcd.ind()
    ...
    pyxcd.w('if anom_crnr_flags_arr[pt_no, 5]:')
    pyxcd.ind()
    pyxcd.w('curr_rule = <int> cp_rules[cp_no, pt_no + n_anom_cols - 1]')
    pyxcd.w('if (curr_rule != n_fuzz_nos) and (abs(<int> (fuzz_no - curr_rule)) > thresh_steep):')
    pyxcd.ind()
    ...
    pyxcd.w('if anom_crnr_flags_arr[pt_no, 6]:')
    pyxcd.ind()
    pyxcd.w('curr_rule = <int> cp_rules[cp_no, pt_no + n_anom_cols]')
    pyxcd.w('if (curr_rule != n_fuzz_nos) and (abs(<int> (fuzz_no - curr_rule)) > thresh_steep):')
    pyxcd.ind()
    ...
    pyxcd.w('if anom_crnr_flags_arr[pt_no, 7]:')
    pyxcd.ind()
    pyxcd.w('curr_rule = <int> cp_rules[cp_no, pt_no + n_anom_cols + 1]')
    pyxcd.w('if (curr_rule != n_fuzz_nos) and (abs(<int> (fuzz_no - curr_rule)) > thresh_steep):')
    pyxcd.ind()
    ...
    assert pyxcd.level == 0, ('Level should be zero instead of %d' %
                              pyxcd.level)
    ...
    pyxcd.w('const np.uint8_t[:, :] anom_crnr_flags_arr,')
    pyxcd.w('const int no_steep_anom_flag,')
    ...
    pyxcd.w('const DT_UL n_anom_cols,')
    pyxcd.w('const int thresh_steep,')
    pyxcd.w('int *gen_mod_cp_err_flag,')
    pyxcd.w(') nogil:')
    ...
    pxdcd.w('const np.uint8_t[:, :] anom_crnr_flags_arr,')
    pxdcd.w('const int no_steep_anom_flag,')
    ...
    pxdcd.w('const DT_UL n_anom_cols,')
    pxdcd.w('const int thresh_steep,')
    pxdcd.w('int *gen_mod_cp_err_flag,')
    pxdcd.w(') nogil')
    pxdcd.ded()
    ...
    pyxcd.w('int curr_l')
    pyxcd.w('DT_UL curr_idxs_ctr, curr_iter_ctr')
    pyxcd.w('DT_UL rand_i, rand_v')
    pyxcd.w('DT_UL max_iters = 1000 * n_pts * max_idxs_ct')
    pyxcd.ded()
    ...
    pyxcd.w('while (curr_idxs_ctr < max_idxs_ct):')
    pyxcd.ind()
    ...
    pyxcd.ind()
    pyxcd.w(r'with gil: print("\n########Too many iterations in '
            r'gen_cp_rules (CP: %d, Fuzz No.: %d, curr_idxs_ctr: %d)!'
            r'########\n" % (j, l, curr_idxs_ctr))')
    pyxcd.w('gen_mod_cp_err_flag[0] = 1')
    pyxcd.w('break')
    pyxcd.ded()
    ...
    pyxcd.w('if (no_steep_anom_flag and ')
    pyxcd.ind()
    pyxcd.w('ret_cont_flag(cp_rules, ')
    pyxcd.ind()
    ...
    pyxcd.ded()
    pyxcd.w('cp_rules_idx_ctr[j, l] = curr_idxs_ctr')
    pyxcd.ded(lev=2)
    ...
    pyxcd.w('DT_UL max_iters = 1000 * max_idxs_ct * n_pts, curr_iter_ct = 0')
